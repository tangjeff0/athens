...
(state.nodeCanvasObject(node,ctx,state.globalScale,state.isShadow),"replace"===nodeCanvasObjectMode)){ctx.restore();return}var r=Math.sqrt(Math.max(0,getVal(node)||1))*state.nodeRelSize+padAmount;ctx.beginPath();ctx.arc(node.x,node.y,r,0,2*Math.PI,!1);ctx.fillStyle=getColor(node)||"rgba(31, 120, 180, 0.92)";ctx.fill();state.nodeCanvasObject&&"after"===nodeCanvasObjectMode&&state.nodeCanvasObject(node,state.ctx,state.globalScale)});ctx.restore()})();return this},emitParticle:function(state,link){link&&
(!link.__photons&&(link.__photons=[]),link.__photons.push({__singleHop:!0}));return this}},stateInit:function(){return{forceLayout:d3Force3d.forceSimulation().force("link",d3Force3d.forceLink()).force("charge",d3Force3d.forceManyBody()).force("center",d3Force3d.forceCenter()).force("dagRadial",null).stop(),engineRunning:!1}},init:function(canvasCtx,state){state.ctx=canvasCtx},update:function(state){state.engineRunning=!1;state.onUpdate();null!==state.nodeAutoColorBy&&autoColorObjects(state.graphData.nodes,
accessorFn__default["default"](state.nodeAutoColorBy),state.nodeColor);null!==state.linkAutoColorBy&&autoColorObjects(state.graphData.links,accessorFn__default["default"](state.linkAutoColorBy),state.linkColor);state.graphData.links.forEach(function(link){link.source=link[state.linkSource];link.target=link[state.linkTarget]});if(!state.isShadow){var linkParticlesAccessor=accessorFn__default["default"](state.linkDirectionalParticles);state.graphData.links.forEach(function(link){var numPhotons=Math.round(Math.abs(linkParticlesAccessor(link)));
numPhotons?link.__photons=_toConsumableArray(Array(numPhotons)).map(function(){return{}}):delete link.__photons})}state.forceLayout.stop().alpha(1).nodes(state.graphData.nodes);var linkForce=state.forceLayout.force("link");linkForce&&linkForce.id(function(d){return d[state.nodeId]}).links(state.graphData.links);var nodeDepths=state.dagMode&&getDagDepths(state.graphData,function(node){return node[state.nodeId]},{nodeFilter:state.dagNodeFilter,onLoopError:state.onDagError||void 0}),maxDepth=Math.max.apply(Math,
...
triggerUpdate:!1},linkLabel:{"default":"name",triggerUpdate:!1},linkHoverPrecision:{"default":4,triggerUpdate:!1},enableNodeDrag:{"default":!0,triggerUpdate:!1},enableZoomPanInteraction:{"default":!0,triggerUpdate:!1},enablePointerInteraction:{"default":!0,onChange:function(_,state){state.hoverObj=null},triggerUpdate:!1},onNodeDrag:{"default":function(){},triggerUpdate:!1},onNodeDragEnd:{"default":function(){},triggerUpdate:!1},onNodeClick:{"default":function(){},triggerUpdate:!1},onNodeRightClick:{triggerUpdate:!1},
onNodeHover:{"default":function(){},triggerUpdate:!1},onLinkClick:{"default":function(){},triggerUpdate:!1},onLinkRightClick:{triggerUpdate:!1},onLinkHover:{"default":function(){},triggerUpdate:!1},onBackgroundClick:{"default":function(){},triggerUpdate:!1},onBackgroundRightClick:{triggerUpdate:!1},onZoom:{"default":function(){},triggerUpdate:!1},onZoomEnd:{"default":function(){},triggerUpdate:!1},onRenderFramePre:{triggerUpdate:!1},onRenderFramePost:{triggerUpdate:!1}},require),aliases:{stopAnimation:"pauseAnimation"},
methods:_objectSpread2({graph2ScreenCoords:function(state,x,y){state=d3Zoom.zoomTransform(state.canvas);return{x:x*state.k+state.x,y:y*state.k+state.y}},screen2GraphCoords:function(state,x,y){state=d3Zoom.zoomTransform(state.canvas);return{x:(x-state.x)/state.k,y:(y-state.y)/state.k}},centerAt:function(state,x$jscomp$0,y,transitionDuration){function getCenter(){var t=d3Zoom.zoomTransform(state.canvas);return{x:(state.width/2-t.x)/t.k,y:(state.height/2-t.y)/t.k}}function setCenter(_ref5){var x=_ref5.x;
_ref5=_ref5.y;state.zoom.translateTo(state.zoom.__baseElem,void 0===x?getCenter().x:x,void 0===_ref5?getCenter().y:_ref5)}return state.canvas?void 0!==x$jscomp$0||void 0!==y?(x$jscomp$0=Object.assign({},void 0!==x$jscomp$0?{x:x$jscomp$0}:{},void 0!==y?{y:y}:{}),transitionDuration?(new TWEEN__default["default"].Tween(getCenter())).to(x$jscomp$0,transitionDuration).easing(TWEEN__default["default"].Easing.Quadratic.Out).onUpdate(setCenter).start():setCenter(x$jscomp$0),this):getCenter():null},zoom:function(state,
k,transitionDuration){return state.canvas?void 0!==k?(transitionDuration?(new TWEEN__default["default"].Tween({k:d3Zoom.zoomTransform(state.canvas).k})).to({k:k},transitionDuration).easing(TWEEN__default["default"].Easing.Quadratic.Out).onUpdate(function(_ref6){state.zoom.scaleTo(state.zoom.__baseElem,_ref6.k)}).start():state.zoom.scaleTo(state.zoom.__baseElem,k),this):d3Zoom.zoomTransform(state.canvas).k:null},zoomToFit:function(state){for(var transitionDuration=1<arguments.length&&void 0!==arguments[1]?
arguments[1]:0,padding=2<arguments.length&&void 0!==arguments[2]?arguments[2]:10,_len=arguments.length,bboxArgs=Array(3<_len?_len-3:0),_key=3;_key<_len;_key++)bboxArgs[_key-3]=arguments[_key];if(_len=this.getGraphBbox.apply(this,bboxArgs))padding=Math.max(1E-12,Math.min(1E12,(state.width-2*padding)/(_len.x[1]-_len.x[0]),(state.height-2*padding)/(_len.y[1]-_len.y[0]))),this.centerAt((_len.x[0]+_len.x[1])/2,(_len.y[0]+_len.y[1])/2,transitionDuration),this.zoom(padding,transitionDuration);return this},
getGraphBbox:function(state){var nodeFilter=1<arguments.length&&void 0!==arguments[1]?arguments[1]:function(){return!0},getVal=accessorFn__default["default"](state.nodeVal);nodeFilter=state.graphData.nodes.filter(nodeFilter).map(function(node){return{x:node.x,y:node.y,r:Math.sqrt(Math.max(0,getVal(node)||1))*state.nodeRelSize}});return nodeFilter.length?{x:[d3Array.min(nodeFilter,function(node){return node.x-node.r}),d3Array.max(nodeFilter,function(node){return node.x+node.r})],y:[d3Array.min(nodeFilter,
function(node){return node.y-node.r}),d3Array.max(nodeFilter,function(node){return node.y+node.r})]}:null},pauseAnimation:function(state){state.animationFrameRequestId&&(cancelAnimationFrame(state.animationFrameRequestId),state.animationFrameRequestId=null);return this},resumeAnimation:function(state){state.animationFrameRequestId||this._animationCycle();return this},_destructor:function(){this.pauseAnimation();this.graphData({nodes:[],links:[]})}},global),stateInit:function(){return{lastSetZoom:1,
forceGraph:new CanvasForceGraph,shadowGraph:(new CanvasForceGraph).cooldownTicks(0).nodeColor("__indexColor").linkColor("__indexColor").isShadow(!0),colorTracker:new ColorTracker__default["default"]}},init:function(domNode,state){domNode.innerHTML="";var container=document.createElement("div");container.classList.add("force-graph-container");container.style.position="relative";domNode.appendChild(container);state.canvas=document.createElement("canvas");state.backgroundColor&&(state.canvas.style.background=
state.backgroundColor);container.appendChild(state.canvas);state.shadowCanvas=document.createElement("canvas");var ctx=state.canvas.getContext("2d"),shadowCtx=state.shadowCanvas.getContext("2d");d3Selection.select(state.canvas).call(d3Drag.drag().subject(function(){if(!state.enableNodeDrag)return null;var obj=state.hoverObj;return obj&&"Node"===obj.type?obj.d:null}).on("start",function(ev){var obj=ev.subject;obj.__initialDragPos={x:obj.x,y:obj.y,fx:obj.fx,fy:obj.fy};ev.active||(obj.fx=obj.x,obj.fy=
obj.y);state.canvas.classList.add("grabbable")}).on("drag",function(ev){var obj=ev.subject,initPos=obj.__initialDragPos,k=d3Zoom.zoomTransform(state.canvas).k,translate={x:initPos.x+(ev.x-initPos.x)/k-obj.x,y:initPos.y+(ev.y-initPos.y)/k-obj.y};["x","y"].forEach(function(c){return obj["f".concat(c)]=obj[c]=initPos[c]+(ev[c]-initPos[c])/k});state.forceGraph.d3AlphaTarget(.3).resetCountdown();obj.__dragged=!0;state.onNodeDrag(obj,translate)}).on("end",function(ev){ev=ev.subject;var initPos=ev.__initialDragPos,
translate={x:ev.x-initPos.x,y:ev.y-initPos.y};void 0===initPos.fx&&(ev.fx=void 0);void 0===initPos.fy&&(ev.fy=void 0);delete ev.__initialDragPos;state.forceGraph.d3AlphaTarget(0).resetCountdown();state.canvas.classList.remove("grabbable");state.isPointerDragging=!1;ev.__dragged&&(delete ev.__dragged,state.onNodeDragEnd(ev,translate))}));state.zoom=d3Zoom.zoom();state.zoom(state.zoom.__baseElem=d3Selection.select(state.canvas));state.zoom.__baseElem.on("dblclick.zoom",null);state.zoom.filter(function(ev){return state.enableZoomPanInteraction?
!ev.button:!1}).scaleExtent([.01,1E3]).on("zoom",function(){var t=d3Zoom.zoomTransform(this);[ctx,shadowCtx].forEach(function(c){resetTransform(c);c.translate(t.x,t.y);c.scale(t.k,t.k)});state.onZoom(_objectSpread2({},t))}).on("end",function(){var t=d3Zoom.zoomTransform(this);state.onZoomEnd(_objectSpread2({},t))});adjustCanvasSize(state);state.forceGraph.onFinishUpdate(function(){d3Zoom.zoomTransform(state.canvas).k===state.lastSetZoom&&state.graphData.nodes.length&&state.zoom.scaleTo(state.zoom.__baseElem,
state.lastSetZoom=4/Math.cbrt(state.graphData.nodes.length))});var toolTipElem=document.createElement("div");toolTipElem.classList.add("graph-tooltip");container.appendChild(toolTipElem);var JSCompiler_object_inline_x_4404=-1E12,JSCompiler_object_inline_y_4405=-1E12;["pointermove","pointerdown"].forEach(function(evType){return container.addEventListener(evType,function(ev){!state.isPointerDragging&&"pointermove"===ev.type&&0<ev.pressure&&[ev.movementX,ev.movementY].some(function(m){return Math.abs(m)>
("touch"===ev.pointerType?1:0)})&&(state.isPointerDragging=!0);var offset=function(el){el=el.getBoundingClientRect();return{top:el.top+(window.pageYOffset||document.documentElement.scrollTop),left:el.left+(window.pageXOffset||document.documentElement.scrollLeft)}}(container);JSCompiler_object_inline_x_4404=ev.pageX-offset.left;JSCompiler_object_inline_y_4405=ev.pageY-offset.top;toolTipElem.style.top="".concat(JSCompiler_object_inline_y_4405,"px");toolTipElem.style.left="".concat(JSCompiler_object_inline_x_4404,
"px")},{passive:!0})});container.addEventListener("pointerup",function(ev){state.isPointerDragging?state.isPointerDragging=!1:requestAnimationFrame(function(){if(0===ev.button)if(state.hoverObj)state["on".concat(state.hoverObj.type,"Click")](state.hoverObj.d,ev);else state.onBackgroundClick(ev);if(2===ev.button)if(state.hoverObj){var fn=state["on".concat(state.hoverObj.type,"RightClick")];fn&&fn(state.hoverObj.d,ev)}else state.onBackgroundRightClick&&state.onBackgroundRightClick(ev)})},{passive:!0});
container.addEventListener("contextmenu",function(ev){if(!state.onBackgroundRightClick&&!state.onNodeRightClick&&!state.onLinkRightClick)return!0;ev.preventDefault();return!1});state.forceGraph(ctx);state.shadowGraph(shadowCtx);var refreshShadowCanvas=throttle__default["default"](function(){clearCanvas(shadowCtx,state.width,state.height);state.shadowGraph.linkWidth(function(l){return accessorFn__default["default"](state.linkWidth)(l)+state.linkHoverPrecision});var t=d3Zoom.zoomTransform(state.canvas);
state.shadowGraph.globalScale(t.k).tickFrame()},800);(this._animationCycle=function animate(){if(state.enablePointerInteraction){var obj=null;if(!state.isPointerDragging){var pxScale=window.devicePixelRatio;(pxScale=0<JSCompiler_object_inline_x_4404&&0<JSCompiler_object_inline_y_4405?shadowCtx.getImageData(JSCompiler_object_inline_x_4404*pxScale,JSCompiler_object_inline_y_4405*pxScale,1,1):null)&&(obj=state.colorTracker.lookup(pxScale.data))}if(obj!==state.hoverObj){var prevObjType=(pxScale=state.hoverObj)?
pxScale.type:null,objType=obj?obj.type:null;if(prevObjType&&prevObjType!==objType)state["on".concat(prevObjType,"Hover")](null,pxScale.d);if(objType)state["on".concat(objType,"Hover")](obj.d,prevObjType===objType?pxScale.d:null);pxScale=obj?accessorFn__default["default"](state["".concat(obj.type.toLowerCase(),"Label")])(obj.d)||"":"";toolTipElem.style.visibility=pxScale?"visible":"hidden";toolTipElem.innerHTML=pxScale;state.hoverObj=obj}refreshShadowCanvas()}clearCanvas(ctx,state.width,state.height);
obj=d3Zoom.zoomTransform(state.canvas).k;state.onRenderFramePre&&state.onRenderFramePre(ctx,obj);state.forceGraph.globalScale(obj).tickFrame();state.onRenderFramePost&&state.onRenderFramePost(ctx,obj);TWEEN__default["default"].update();state.animationFrameRequestId=requestAnimationFrame(animate)})()},update:function(state){}});module.exports=require}
