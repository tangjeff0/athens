...
!exceptions_1_1.done&&(_a=exceptions_1.return)&&_a.call(exceptions_1)}finally{if(e_1)throw e_1.error;}}}e_1=event.user;if(!session.userAgent){event=event.request?event.request.headers:{};for(var key in event)if("user-agent"===key.toLowerCase()){var userAgent=event[key];break}}session.update(tslib_1.__assign(tslib_1.__assign({},crashed&&{status:types_1.SessionStatus.Crashed}),{user:e_1,userAgent:userAgent,errors:session.errors+Number(errored||crashed)}))};BaseClient.prototype._sendSession=function(session){this._getBackend().sendSession(session)};
