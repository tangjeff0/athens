...
" due to too many requests.",status:429}):this._buffer.add(new utils_1.SyncPromise(function(resolve,reject){var request=new XMLHttpRequest;request.onreadystatechange=function(){if(4===request.readyState){var headers={"x-sentry-rate-limits":request.getResponseHeader("X-Sentry-Rate-Limits"),"retry-after":request.getResponseHeader("Retry-After")};_this._handleResponse({requestType:sentryRequest.type,response:request,headers:headers,resolve:resolve,reject:reject})}};request.open("POST",sentryRequest.url);
for(var header in _this.options.headers)_this.options.headers.hasOwnProperty(header)&&request.setRequestHeader(header,_this.options.headers[header]);request.send(sentryRequest.body)}))};return XHRTransport}(require("module$node_modules$$sentry$browser$dist$transports$base").BaseTransport);exports.XHRTransport=global}
